...
def main(argv: list[str] | None = None) -> int:
    ...
    features_dir = Path(args.features_dir) if args.features_dir else run_dir / "features" / args.model
    model_dir = Path(args.model_dir) if args.model_dir else run_dir / "models" / args.model
    ...
    print(json.dumps({"format": fmt, "model": args.model, "model_dir": str(model_dir), "metrics": metrics}, ensure_ascii=False, sort_keys=True))
    return 0
