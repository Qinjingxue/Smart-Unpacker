...
class _RuntimeTeacherContext:
    def __init__(self, *, config: dict[str, Any], workspace: Path | None, recovery_mode: str, budget: dict[str, Any]):
        ...
        self.repair_inspection_service = RepairInspectionService(self.full_config)
    ...
    def _diagnose_state(
        ...
        *,
        ...
        if diagnosis:
            return sanitize_diagnosis_hgt(diagnosis)
        fallback = _diagnosis_hgt_from_row(row)
        if fallback:
            fallback = sanitize_diagnosis_hgt(fallback)
    ...
    def _decision_sample(
        ...
        *,
        ...
        sample = PolicyGraphTrainingSample(
            ...
            format=job.format,
            graph=sanitize_training_graph(repair_graph.graph.to_dict()),
            current_node_id=repair_graph.graph.current_node_id,
            best_node_id=repair_graph.graph.best_node_id,
            actions=actions,
            memory=list(memory[-64:]),
            diagnosis_hgt=sanitize_diagnosis_hgt(diagnosis),
            current_recovery=sanitize_recovery(current.recovery if current is not None else {}),
            best_recovery=sanitize_recovery(best.recovery if best is not None else {}),
...
def _memory_entry(*, step: int, action: PolicyAction, recovery: PolicyRecoverySnapshot, previous_score: float, best_updated: bool, stale: int, repair_graph: PolicyRepairGraph) -> dict[str, Any]:
    ...
    return {
        ...
        "current_recovery": sanitize_recovery(recovery.to_dict()),
