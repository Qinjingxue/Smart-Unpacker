...
def build_policy_graph_rows(raw_rows: list[dict[str, Any]], *, format_name: str = "zip") -> list[PolicyGraphTrainingSample]:
    ...
    for row_index, row in enumerate(raw_rows):
        ...
        for item_index, item in enumerate(history_items):
            ...
            if not loop:
                continue
            graph = sanitize_training_graph(loop.get("graph") if isinstance(loop.get("graph"), dict) else {})
            ...
            best_recovery = sanitize_recovery(last_round.get("best_seen_recovery") if isinstance(last_round.get("best_seen_recovery"), dict) else {})
            ...
            samples.append(PolicyGraphTrainingSample(
                ...
                actions=actions,
                memory=[sanitize_memory_entry(entry) for entry in rounds if isinstance(entry, dict)],
                diagnosis_hgt=sanitize_diagnosis_hgt(last_round.get("diagnosis_hgt") if isinstance(last_round.get("diagnosis_hgt"), dict) else {}),
                current_recovery=sanitize_recovery(last_round.get("current_recovery") if isinstance(last_round.get("current_recovery"), dict) else {}),
