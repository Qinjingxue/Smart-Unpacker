...
def _transition_world_sample(transition: PolicyGraphTransitionSample) -> PolicyWorldTrainingSample:
    return PolicyWorldTrainingSample(
        ...
        observed_delta=transition.observed_delta or observed_delta(transition.graph_before, transition.graph_after),
...
def _apply_immediate_repeat_penalty(actions: list[PolicyAction], graph: dict[str, Any]) -> list[PolicyAction]:
    incoming_module, incoming_family = incoming_module_family(graph)
...
def _apply_module_history_feedback(actions: list[PolicyAction], graph: dict[str, Any]) -> list[PolicyAction]:
    module_history, family_history = module_history_stats(graph)
    ...
    for action in actions:
        ...
        penalty = history_penalty(exact, exact=True)
        if penalty <= 0.0:
            penalty = history_penalty(family_stats, exact=False)
        if penalty <= 0.0:
            output.append(action)
            continue
        features = {
...
def _apply_current_node_retry_penalty(actions: list[PolicyAction], graph: dict[str, Any]) -> list[PolicyAction]:
    tried_modules, tried_families = current_node_tried_modules(graph)
...
def observed_delta(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
