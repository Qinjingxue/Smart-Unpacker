...
from repair_training.core.repair_policy_transformer.tensorize import ACTION_FEATURE_DIM, MEMORY_FEATURE_DIM, NODE_FEATURE_DIM, require_torch
...
class RepairGraphMemoryPolicyTransformer:
    def __new__(cls, *args, **kwargs):
        ...
        class _Model(nn.Module):
            ...
            def forward(self, node_x, memory_x, action_x):
                node_h = self.graph_encoder(self.node_in(node_x).unsqueeze(0)).squeeze(0)
                memory_h = self.memory_encoder(self.memory_in(memory_x).unsqueeze(0)).squeeze(0)
                context = node_h if memory_h.numel() == 0 else __import__("torch").cat([node_h, memory_h], dim=0)
