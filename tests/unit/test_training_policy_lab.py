...
def test_episode_collector_emits_patch_state_episode(tmp_path):
    ...
    assert any(t.selected_action and t.selected_action.action_type == "apply_patch" for t in restored.transitions)
...
def test_value_labeler_propagates_delayed_reward_to_early_actions():
    ...
    by_state = {row["state_digest"]: row for row in rows if row["action_type"] == "apply_patch"}
...
def test_value_labeler_prefers_undo_when_child_state_is_worse():
    episode = TrainingEpisode(
        ...
        transitions=[
            ...
            TrainingTransition(
                ...
                state_digest="child",
                patch_depth=1,
                available_actions=[TrainingAction(action_type="undo_patch")],
                selected_action=TrainingAction(action_type="undo_patch", metadata={"target_state_digest": "root"}),
    ...
    undo = next(row for row in rows if row["action_type"] == "undo_patch")
    ...
    give_up_episode = TrainingEpisode(
        ...
        transitions=[
            TrainingTransition(
                ...
                state_digest="empty",
                patch_depth=0,
                selected_action=TrainingAction(action_type="give_up"),
...
def _edge(source, target, candidate_id, before, after):
    return TrainingTransition(
        ...
        available_actions=[TrainingAction(action_type="apply_patch", candidate_id=candidate_id)],
        selected_action=TrainingAction(action_type="apply_patch", candidate_id=candidate_id),
