...
filesystem / relations / rename
  -> contracts
  -> smart_unpacker_native narrow helpers
...
| 领域 | 公开入口 | 职责 |
| ---- | -------- | ---- |
| CLI | `smart_unpacker.app.cli.main` | 命令行入口。 |
...
```python
from smart_unpacker.some_domain.internal import ...
```
...
```python
from smart_unpacker.coordinator.runner import PipelineRunner  # inside watch scheduler
```

`watch` 不直接绑定 coordinator runner，由 app 注入 runner factory。

```python
from smart_unpacker.detection.pipeline.processors.modules... import SOME_RULE_DEFAULT
```
