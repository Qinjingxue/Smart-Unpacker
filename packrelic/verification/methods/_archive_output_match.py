...
from packrelic.support.path_names import clean_relative_archive_path, normalize_match_path
from packrelic.verification.result import FileVerificationObservation, VerificationIssue
...
def _index_output_files(files: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for raw in files:
        ...
        if not path or ".packrelic/" in path:
            continue
        item = dict(raw)
        item["path"] = path
