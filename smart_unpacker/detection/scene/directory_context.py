...
def detect_scene_context_for_directory(target_dir: str, rules: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return cached_value(
        ...
        lambda: context_from_markers(
            norm_target,
            collect_scene_markers_from_snapshot(DirectoryScanner(norm_target).scan(), effective_rules),
            effective_rules,
        ),
    )
