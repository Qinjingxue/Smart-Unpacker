...
class PasswordResolver:
    ...
    def resolve(self, archive_path: str, fact_bag: FactBag | None = None) -> PasswordResolution:
        ...
        test_result, error_text = self.password_manager.test_without_password(archive_path)
        ...
        if has_definite_wrong_password(error_text) or "cannot open encrypted archive" in error_text:
            password, result, error = self.password_manager.find_working_password(archive_path)
