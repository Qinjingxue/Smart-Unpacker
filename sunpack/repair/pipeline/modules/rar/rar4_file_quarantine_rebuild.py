...
class Rar4FileQuarantineRebuild:
    spec = RepairModuleSpec(
        name="rar4_file_quarantine_rebuild",
        formats=("rar",),
        ...
        routes=(
            RepairRoute(
                ...
                require_any_flags=("rar4", "file_block_bad", "damaged", "crc_error", "checksum_error", "data_error"),
                require_any_failure_kinds=("corrupted_data", "data_error", "checksum_error"),
                reject_any_flags=("wrong_password", "encrypted", "rar5"),
                base_score=0.9,
            ),
        ),
    )
    ...
    def can_handle(self, job: RepairJob, diagnosis: RepairDiagnosis, config: dict) -> float:
        ...
        if flags & {"wrong_password", "encrypted", "rar5"}:
            return 0.0
        if "rar4" in flags and flags & {"file_block_bad", "damaged", "crc_error", "checksum_error", "data_error"}:
            return 0.94
        return 0.0
    ...
    def generate_candidates(self, job: RepairJob, diagnosis: RepairDiagnosis, workspace: str, config: dict):
        ...
        return candidates_from_native_result(
            ...
            job,
            diagnosis,
            native_key="native_rar4_file_quarantine",
            format_hint="rar",
            partial_default=True,
            default_confidence=0.72,
            default_message="RAR4 file quarantine produced a candidate",
        )

    def _run_native(self, job: RepairJob, workspace: str, config: dict) -> dict:
        deep = config.get("deep") if isinstance(config.get("deep"), dict) else {}
        return dict(_native_rar_file_quarantine(
...
register_repair_module(Rar4FileQuarantineRebuild())
