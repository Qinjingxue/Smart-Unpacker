...
AssessmentScoreFn = Callable[[dict[str, Any]], float]
...
class RepairBeamLoop:
    ...
    def expand_round(self, states: list[RepairBeamState], *, round_index: int) -> RepairBeamRound:
        ...
        assessed = [
            _with_assessment(item, self.assess(item), self.score_assessment)
...
def _with_assessment(
    ...
    assessment: VerificationResult | dict[str, Any] | None,
    score_assessment: AssessmentScoreFn,
) -> RepairBeamCandidate:
    payload = _assessment_payload(assessment)
    if not payload:
        return item
    score = _verification_score_with_assessment(item.score, payload, score_assessment)
    return RepairBeamCandidate(
        state=item.state,
        candidate=item.candidate,
        analyze=item.analyze,
...
def _verification_score_with_assessment(
    base_score: float,
    assessment: dict[str, Any],
    score_assessment: AssessmentScoreFn,
) -> float:
    verification_score = score_assessment(assessment)
